...
import numpy as np
from keras_med_io.batchgenerators.augmentations.utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, \
    interpolate_img, \
    rotate_coords_2d, rotate_coords_3d, scale_coords, resize_segmentation
from skimage.transform import resize
from keras_med_io.batchgenerators.augmentations.crop_and_pad_augmentations import random_crop as random_crop_aug
...
def augment_spatial_nocrop(data, seg, ndim,
                    ...
                    border_mode_seg='constant', border_cval_seg=0, order_seg=0, p_el_per_sample=1,
                    p_scale_per_sample=1, p_rot_per_sample=1):
    '''
    Removed cropping from previous `augment_spatial` func.
    Notes:
        Assumes shape (batch, n_channels, x, y) or (batch, n_channels, x, y, z)
        Assumes channels_first
    ...
    if seg is not None:
        seg_result = np.zeros(seg.shape, dtype = np.float32)
    data_result = np.zeros(data.shape, dtype = np.float32)

    for sample_id in range(data.shape[0]):
        coords = create_zero_centered_coordinate_mesh(base_shape)

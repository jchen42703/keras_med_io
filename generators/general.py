...
class NormalizationGenerator(BaseGenerator):
    """
    Streams directly from the directories and applies normalization and data augmentation.
    ** CHANNELS_LAST
    ...
    Attributes:
        ...
        transforms_args: a dictionary mapping the "keras_med_io.utils.io_func.transforms" arguments to the desired values
            It should not include volume, and segmentation as arguments. (optional, default: None)
            ** Note: If `transforms_args` is None, then there is no data augmentation.
                     Also, transforms_args requires the `dims` argument in this generator's case.
        shuffle: boolean
    """
    def __init__(self, list_IDs, data_dirs, batch_size, n_channels, n_classes, normalize_mode = "normalize",
                 norm_range = [0,1], transforms_args = None, shuffle = True):
    ...
    def data_gen(self, list_IDs_temp):
        ...
        for id in list_IDs_temp:
            ...
            x_train = normalization(x_train, self.normalize_mode, self.norm_range)
            ...
            # data augmentation
            imgs_x.append(x_train), imgs_y.append(y_train)

        input_data, seg_masks = np.stack(imgs_x), np.stack(imgs_y)
...
class NormalizedPatchGenerator(PosRandomPatchExtractor, NormalizationGenerator):
    """
    Normalized patchwise patch generator
    Adds the option to balance your data based on a specified number of positive patches you want to have in each batch
    New params:
        * n_pos: the number of images in batch to contain a positive class
    ** CHANNELS_LAST
    ** LOADS DATA WITH Nibabel instead of SimpleITK
    ...
    def __init__(self, list_IDs, data_dirs, batch_size, patch_shape, n_channels, n_classes,
                 ...
                 transforms_args = None, shuffle = True):

        NormalizationGenerator.__init__(self, list_IDs = list_IDs, data_dirs = data_dirs, batch_size = batch_size,
